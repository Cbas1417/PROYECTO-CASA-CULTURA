  ...
  if (inscripciones.length === 0) {
    ...
    cuerpoTabla.innerHTML = inscripciones.map(insc => `
      <tr>
        ...
        <td class="contacto-cell">${insc.contacto || insc.correo || ''}</td>
        <td>${insc.nombre || ''}</td>
        <td>${insc.edad || ''}</td>
...
function buscarInscripciones(termino) {
  ...
  const resultados = inscripciones.filter(insc => 
    (insc.programaNombre && insc.programaNombre.toLowerCase().includes(term)) ||
    (insc.contacto && insc.contacto.toLowerCase().includes(term)) ||
    (insc.correo && insc.correo.toLowerCase().includes(term)) ||
    (insc.nombre && insc.nombre.toLowerCase().includes(term)) ||
    (insc.edad && insc.edad.toString().includes(term)) ||
    (insc.fecha && insc.fecha.toLowerCase().includes(term))
  );
  ...
  if (resultados.length === 0) {
    ...
    cuerpoTabla.innerHTML = resultados.map(insc => `
      <tr>
        ...
        <td class="contacto-cell">${resaltarCoincidencia(insc.contacto || insc.correo || '', termino)}</td>
        <td>${resaltarCoincidencia(insc.nombre || '', termino)}</td>
        <td>${resaltarCoincidencia(insc.edad ? insc.edad.toString() : '', termino)}</td>
        <td class="fecha-cell">${insc.fecha}</td>
        <td>
          <button class="btn-eliminar" onclick="eliminarInscripcion(${insc.id})">
            <i class="fas fa-trash-alt"></i> Eliminar
...
function mostrarMensaje(mensaje, tipo) {
  ...
  `;
  mensajeEl.style.backgroundColor = tipo === 'success' ? '#28a745' : '#dc3545';
